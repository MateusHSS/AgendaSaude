...
<body>
    ...
    <main>
        <div class="row">
            <div class="container">
                ...
                $sqlSomaUsuarios = $connect->prepare("SELECT COUNT(*) AS total FROM usuario");
                $sqlSomaUsuarios->execute();
                $resultSoma = $sqlSomaUsuarios->get_result();
                $resSoma = $resultSoma->fetch_assoc();

            ?>
                    <div class="row">
                        <h5 class='left'>Usuários<i
                    ...
                    <div class="row z-depth-2" id="listagem_paciente">
                        <table class="centered striped">
                            ...
                            <tbody>
                                <?php
                $sqlListaUsuarios = $connect->prepare("SELECT usuario.* FROM usuario");
                $sqlListaUsuarios->execute();

                $resultUsuarios = $sqlListaUsuarios->get_result();
                    ...
                    if($resultUsuarios->num_rows < 0){
                        ...
                        while($resUsuarios = $resultUsuarios->fetch_assoc()){
                            ?>
                                <tr>
                                    <td><?php echo $resUsuarios['id_usuario'] ?></td>
                                    <td><?php echo $resUsuarios['user'] ?></td>
                                    <td>
                                        <button class="btn btn-small button_edita white red-text modal-trigger"
                                            data-target="edita" data-id="<?php echo $resUsuarios['id_usuario'] ?>"><i
                                                class="material-icons">edit</i></button>
                                        <button class="btn btn-small button_exclui white red-text modal-trigger"
                                            data-target="exclui" data-id="<?php echo $resUsuarios['id_usuario'] ?>"><i
        ...
        <div id="edita" class="modal">
            <div class="modal-content">
                ...
                <form method='post' class='center' id="atualiza_dados_usuario" data-id="">
                    <div class="row">
    ...
    <script src="../../js/listas/lista_usuarios.js"></script>
</body>
