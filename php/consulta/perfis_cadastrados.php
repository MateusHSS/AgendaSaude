...
<body>
    ...
    <main>
        <div class="row">
            <div class="container">
                ...
                $sqlSomaPerfis = $connect->prepare("SELECT COUNT(*) AS total FROM perfil");
                $sqlSomaPerfis->execute();
                $resultSoma = $sqlSomaPerfis->get_result();
                $resSoma = $resultSoma->fetch_assoc();

            ?>
                    <div class="row">
                        <h5 class='left'>Perfis<i
                    ...
                    <div class="row z-depth-2" id="listagem_paciente">
                        <table class="centered striped">
                            ...
                            <tbody>
                                <?php
                $sqlListaPerfis = $connect->prepare("SELECT * FROM perfil");
                $sqlListaPerfis->execute();

                $resultPerfis = $sqlListaPerfis->get_result();

                    if($resultPerfis->num_rows == 0){
                        ?>
                                <tr>
                                    <td colspan="4">
                        ...
                        while($resPerfis = $resultPerfis->fetch_assoc()){
                            ?>
                                <tr>
                                    <td><?php echo $resPerfis['descricao'] ?></td>
                                    <td>
                                        <button class="btn btn-small button_edita white red-text modal-trigger"
                                            data-target="edita" data-id="<?php echo $resPerfis['id_perfil'] ?>"><i
                                                class="material-icons">edit</i></button>
                                        <button class="btn btn-small button_exclui white red-text modal-trigger"
                                            data-target="exclui" data-id="<?php echo $resPerfis['id_perfil'] ?>"><i
        ...
        <div id="edita" class="modal">
            <div class="modal-content">
                ...
                <form method='post' class='center' id="atualiza_dados_perfil" data-id="">
                    <div class="row">
    ...
    <script src="../../js/listas/lista_perfis.js"></script>
</body>
